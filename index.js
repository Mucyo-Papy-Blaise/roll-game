...
// Define the weapons with their power levels
const weapons = [
  { name: 'stick', power: 5 },
...
// Define the monsters with their levels and health
const monsters = [
...
// Define the locations and what happens there
const locations = [
...
// Initialize buttons with their default actions
button1.onclick = goStore;
button2.onclick = goCave;
button3.onclick = fightDragon;

// Update the game display based on the location
...
// Buy the next weapon for 30 gold
function buyWeapon() {
...
// Define functions for fighting each monster
function fightSlime() {
  fighting = 0;
  goFight();
}
...
// Start the fight with the selected monster
function goFight() {
  update(locations[3]);
...
// Calculate how much damage the monster does to the player
...
// Player dodges the monster's attack
function dodge() {
...
// Go to the secret easter egg location
function easterEgg() {
  update(locations[7]);
}

// Functions for choosing a number in the easter egg game
function pickTwo() {
  pick(2);
}
...
// Pick a number and see if it matches a random number
function pick(guess) {
  const numbers = [];
